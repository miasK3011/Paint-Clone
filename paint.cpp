...
// Funcao para armazenar um vertice na forma do inicio da lista de formas
// geometricas Armazena sempre no inicio da lista
...
int main(int argc, char **argv) {
  ...
  glutInitDisplayMode(
      GLUT_DOUBLE | GLUT_RGB); // Selecao do Modo do Display e do Sistema de cor
  glutInitWindowSize(width, height); // Tamanho da janela do OpenGL
  glutInitWindowPosition(100, 100);  // Posicao inicial da janela do OpenGL
  glutCreateWindow(
      "Computacao Grafica: Paint"); // Da nome para uma janela OpenGL
  init();                           // Chama funcao init();
  glutReshapeFunc(reshape);         // funcao callback para redesenhar a tela
  glutKeyboardFunc(keyboard);       // funcao callback do teclado
  glutMouseFunc(mouse);             // funcao callback do mouse
  glutPassiveMotionFunc(
      mousePassiveMotion);  // fucao callback do movimento passivo do mouse
...
void display(void) {
  ...
  draw_text_stroke(0, 5, "(x:" + to_string(m_x) + ", y:" + to_string(m_y) + ")",
                   0.2);
...
void drawFormas() {
  ...
  if (click1) {
    ...
    if (modo == RET)
      drawQuadrilatero(x_1, y_1, m_x, m_y);
    if (modo == TRI)
      drawTriangulo(x_1, y_1, m_x, m_y, x_2, y_2);
  ...
  for (forward_list<forma>::iterator f = formas.begin(); f != formas.end();
       f++) {
    switch (f->tipo) {
    case LIN: {
      int i = 0, x[2], y[2];
      // Percorre a lista de vertices da forma linha para desenhar
      for (forward_list<vertice>::iterator v = f->v.begin(); v != f->v.end();
           v++, i++) {
        x[i] = v->x;
        y[i] = v->y;
      }
...
void reflexao(bool x, bool y) {
  ...
  if (x) {
    for (auto it = formas.front().v.begin(); it != formas.front().v.end();
         ++it) {
      it->x *= -1;
    }
  } else if (y) {
...
void cisalhamento(float c) {
  ...
  float matriz[2][2] = {{1.0f, c}, {0.0f, 1.0f}};
...
/*
        Tentei implementar da forma que esta nos slides, mas a unica que
   funcionou foi essa implementacao no site:
